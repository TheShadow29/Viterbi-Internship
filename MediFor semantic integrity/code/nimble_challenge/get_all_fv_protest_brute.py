...
def worker_task(img_top_dir, transformer, slice_id, bbox_list_dict):
    # global count
    # Convention followed :
    ...
    while True:
        if q1.qsize() < 100:
            ...
            bbox_list = bbox_list_dict[img_file_name[:-4]]
...
if __name__ == '__main__':
    ...
    for i in img_file_names:
        ...
        # pdb.set_trace()
    # pdb.set_trace()
    workers = [mp.Process(target=worker_task, args=(img_top_dir, transformer, slice_id, bbx_list_dict)) for i in range(num_process)]
    for w in workers:
        w.start()
        total_num = 0
